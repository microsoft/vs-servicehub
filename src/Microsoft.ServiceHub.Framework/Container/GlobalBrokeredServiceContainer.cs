...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
		...
		lock (this.syncObject)
		{
			foreach (KeyValuePair<ServiceMoniker, ServiceRegistration> service in services)
			{
				if (!this.registeredServices.TryGetValue(service.Key, out ServiceRegistration? existingRegistration))
				{
					...
					if (this.TryRemoveNilMarkers_NoLock(service.Key))
					{
						(availabilityChangedMonikers ??= ImmutableHashSet.CreateBuilder<ServiceMoniker>()).Add(service.Key);
					}
	...
	/// Overriding methods should register services using <see cref="RegisterServices(IReadOnlyDictionary{ServiceMoniker, ServiceRegistration})"/>.
	/// </para>
	/// <para>This method is always invoked after <see cref="LoadAllRegistrationsCore"/>, and therefore this method only needs to load additional registrations that require asynchronous discovery.</para>
	/// <para>Derived classes do not need to worry about concurrency; this method will only be invoked once.</para>
	/// </remarks>
	/// <returns>A task that completes when any asynchronous registration loading has completed.</returns>
	protected virtual ValueTask LoadAllRegistrationsCoreAsync()
	{
		return default;
	}
	...
	/// <returns><see langword="true"/> if any nil markers were removed; <see langword="false"/> otherwise.</returns>
	/// <remarks>
	/// Callers should already hold a lock on <see cref="syncObject"/> when calling this method.
	/// </remarks>
	private bool TryRemoveNilMarkers_NoLock(ServiceMoniker registeredMoniker)
	{
	...
	private struct ProffererId : IEquatable<ProffererId>
	{
		...
		public override int GetHashCode() =>
			...
			this.weakRef is not null ? this.weakRef.GetHashCode() :
			0;
	}
