...
public abstract class ProxyBase : IDisposableObservable, INotifyDisposable, IClientProxy, IJsonRpcLocalProxy
{
	...
	public event EventHandler? Disposed
	{
		add
		{
			object? oldValue = this.disposedHandlers;
			while (true)
			{
				if (oldValue == DisposedSentinel)
				{
					value?.Invoke(this, EventArgs.Empty);
					return;
				}

				object? newValue = Delegate.Combine((EventHandler?)oldValue, value);
				object? prevValue = Interlocked.CompareExchange(ref this.disposedHandlers, newValue, oldValue);
		...
		remove
		{
			...
			while (true)
			{
				...
				object? newValue = Delegate.Remove((EventHandler?)oldValue, value);
				object? prevValue = Interlocked.CompareExchange(ref this.disposedHandlers, newValue, oldValue);
				if (prevValue == oldValue)
				{
					return;
