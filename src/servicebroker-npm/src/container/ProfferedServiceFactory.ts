		...
		try {
			if (service) {
				...
				const disposeConnection = () => {
					pipePair.second.off('finish', disposeConnection)
					pipePair.second.off('close', disposeConnection)
					connection.dispose()
				}
				pipePair.second.once('finish', disposeConnection)
				pipePair.second.once('close', disposeConnection)
