...
The service contract and all containing types must be `partial` so the Microsoft.ServiceHub source generator can register a resilient proxy. Every inherited instance member must also have a supported shape. Methods may return `Task`, `Task<T>`, `ValueTask`, `ValueTask<T>`, `IAsyncEnumerable<T>`, or `void`; `void` methods are treated as notifications. Observer-subscription methods may return `Task<IDisposable>` or `ValueTask<IDisposable>` when they accept an <xref:System.IObserver`1> and end with a <xref:System.Threading.CancellationToken> parameter. Events must use a `void`-returning delegate with exactly two parameters: an `object` sender followed by a non-`ref` event argument. Properties, indexers, and other unsupported member shapes prevent resilient proxy mapping generation.

Initial unavailability still returns `null`. After a proxy has been returned, calls wait for an in-progress replacement acquisition, but throw <xref:Microsoft.ServiceHub.Framework.BrokeredServiceUnavailableException> when the broker confirms that the service is unavailable.
